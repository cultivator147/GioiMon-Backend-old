...
public class StoriesRepository extends BaseRepository {
    ...
    public Page<SampleStoryDTO> getFilteredListStories(long categoryId, int writingState, int page, int size, String sortBy){
        StringBuilder sqlBuilder = new StringBuilder("""
                SELECT  T1.ID AS ID, T1.TITLE AS TITLE, T1.LINK AS LINK, T1.CATEGORY_ID AS CATEGORY_ID, T1.LAST_UPDATE_DATE
                FROM (
                SELECT S.ID AS ID, S.TITLE AS TITLE, S.LINK AS LINK, S.LAST_UPDATE_DATE, SC.CATEGORY_ID AS CATEGORY_ID
                FROM stories S
